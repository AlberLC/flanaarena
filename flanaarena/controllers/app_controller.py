...
from services import champion_fetcher
from services.lcu import Lcu


class AppController[T: GuiApp]:
    def __init__(self, app: T, session: requests.Session) -> None:
        self._app = app
        self._window = self._app.window
        self._gui = self._app.gui
        self._session = session
        self._lcu = Lcu(self._session)
        ...
        self._gui.clear_borders_action.triggered.connect(self._lcu.clear_borders)
        self._gui.clear_tokens_action.triggered.connect(self._lcu.clear_tokens)
        self._gui.check_auto_accept.toggled.connect(self._save_config)
        self._gui.check_auto_ryze.toggled.connect(self._save_config)

    def _load(self) -> None:
        self._champions = champion_fetcher.fetch_champions(self._session)
    ...
    def _run_socket_listener(self) -> None:
        self._lcu.connect_websocket()
        self._lcu_socket_connected_event.set()

        for event_data, event_type, uri in self._lcu.iter_websocket_events():
            if constants.LCU_CHAMPION_SELECTED_URI_PART in uri:
                ...
                if (
                    ...
                    self._lcu.select_champion(event_data['cellId'], constants.RYZE_ID)
    ...
    def _start(self) -> None:
        self._lcu.load_credentials()
        self._gui.show_profile_widgets_signal.emit()
    ...
    def _update_missions_count(self) -> None:
        ...
        while True:
            try:
                missions_count = self._lcu.fetch_missions_count()
